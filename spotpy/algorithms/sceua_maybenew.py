...
class sceua(_algorithm):
    ...
    def simulate(self, id_params_tuple):
        ...
        if not self.repeat.phase:  # burn-in
            ...
            self.npg, self.nopt, self.ngs, self.nspl, self.nps, self.bl, self.bu, self.status, self.stochastic_parameters, repetitions = sce_vars
            ...
            for loop in range(self.nspl):
                ...
                snew, fnew, icall, simulation = self._cceua(s, sf, icall, repetitions)
                likes.append(fnew)
                pars.append(snew)
                ...
                if icall > repetitions:
                    ...
                    return igs, likes, pars, sims, cx, cf, k1, k2, icall
    ...
    def sample(self, repetitions, ngs=20, kstop=100, pcento=0.0000001, peps=0.0000001):
        ...
        while icall < repetitions and gnrng > peps and criter_change_pcent > pcento and proceed == True:
            ...
            remaining_runs = repetitions - icall
            if remaining_runs <= self.ngs:
                self.ngs = remaining_runs-1
                proceed = False
            print(remaining_runs)
            sce_vars = [self.npg, self.nopt, self.ngs, self.nspl,
                        self.nps, self.bl, self.bu, self.status, self.stochastic_parameters, repetitions]
            param_generator = ((rep, x, xf, icall, cx, cf, sce_vars)
                               for rep in range(int(self.ngs)))
            for igs, likes, pars, sims, cx, cf, k1, k2, icall in self.repeat(param_generator):
                #icall += len(likes)
                x[k2, :] = cx[k1, :]
                xf[k2] = cf[k1]
                for i in range(len(likes)):
                    if not self.status.stop and icall < repetitions:    
    ...
    def _cceua(self, s, sf, icall, repetitions):
        ...
        fnew = like
        icall += 1
        if self.status.stop and not icall < repetitions:
